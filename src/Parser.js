  ...
  async parse(tokens, top = true) {
    ...
    for (i = 0; i < l; i++) {
      ...
      switch (token.type) {
        ...
        case "paragraph": {
          out += await this.renderer.paragraph(
            await this.parseInline(token.tokens)
          );
          continue;
        }
  ...
  async parseInline(tokens, renderer) {
    ...
    for (i = 0; i < l; i++) {
      ...
      switch (token.type) {
        ...
        case "link": {
          out += await renderer.link(
            token.href,
            token.title,
            await this.parseInline(token.tokens, renderer)
          );
          break;
        }
        case "image": {
        ...
        case "strong": {
          out += await renderer.strong(
            await this.parseInline(token.tokens, renderer)
          );
          break;
        }
        case "em": {
          out += await renderer.em(
            await this.parseInline(token.tokens, renderer)
          );
          break;
        }
